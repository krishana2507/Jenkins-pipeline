pipeline {
    ...
    stages {
        ...
        stage('Checkout OAS Repository') {
            steps {
                script {
                    // Check out the repository containing the OAS file
                    dir('oas-repo') {
                        git url: params.OAS_Repo_GIT_URL, branch: params.OAS_Repo_GIT_Branch
                    }
                }
            }
        }
        stage('Read and Process YAML') {
            steps { 
                script {
                    // Read the main config.yaml file
                    def configYamlPath = params.Configuration_Yaml_Path
                    def configContent = readFile(configYamlPath).trim()
                    echo "Main Config YAML Content:\n${configContent}"
                    
                    def config = readYaml text: configContent

                    // Override oas_file_path to use petstore.yaml from the OAS repository
                    def oasFilePath = "oas-repo/petstore.yaml".trim()
                    echo "Using OAS file from different repo: ${oasFilePath}"
                    ...
                    // Append global and plugin configurations
                    // (The rest of the pipeline code is unchanged)
                }
            }
        }
